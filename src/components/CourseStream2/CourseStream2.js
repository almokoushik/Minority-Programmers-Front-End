...
const CourseStream = (props) => {
    console.log("yjbscuyezhtvcnkvdtxvdikrcdiedicxrcdxkvrclh")
    const [userData, setUserData] = useContext(UserContext)
    console.log("This is CourseStream 20", userData)

    const { id } = useParams()
    const [data, setData] = useState([])
    useEffect(() => {
        setData(course.courses.filter(singleCourse => parseInt(singleCourse.course_id) === parseInt(userData.course)))
    }, [])

    console.log("This is COurse Stream 22", data[0])
